...
import { resolveIdentity, resolveEntryway, isEntryway, type ResolveDeps } from './resolve.js';
 ...
 * a handle or DID (identity first, `login_hint` sent) OR a provider entryway such
 * as `https://bsky.social` (server first, no hint — the DID comes back in the
 * token). croft-pwa/docs/DESIGN.md § Flows › Sign in.
 */
...
export async function completeAuthorization(
  ...
  const pds = pending.did ? pending.pds : await new RepoClient(cfg.fetchImpl ? { fetchImpl } : {}).resolvePds(did);
