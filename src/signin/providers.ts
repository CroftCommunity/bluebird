// The sign-in sheet's provider registry — croft-pwa/docs/DESIGN.md § Flows › Sign in,
// ported from croft-pwa/src/signin/providers.ts (the reference implementation).
//
// A Croft app has no accounts of its own. Every identity belongs to a server
// someone else runs, and the sheet's job is to route you to one — which is why
// this is a list of real providers with visibly different rules rather than a
// single "Sign in with Bluesky" button. The sheet teaches the shape of the
// network by showing it.
//
// EVERY FACT BELOW WAS PROBED, not inferred (forage, 2026-08-26..29; re-probed
// here by tests/live/signin-providers.live.spec.ts):
...
// is not a PDS (the host is `blacksky.app`); `eurosky.tech` and
// `portal.eurosky.tech` are EuroSky's site and portal, the PDS is
// `eurosky.social`; `mu.social` is a Mastodon server (ActivityPub, 403 on every
// atproto endpoint); `muni.town` is not a PDS.
...
// The SHAPE: the front page is capped, and everything else reaches the same
// code path through "Another provider", which takes a handle on any atproto
// host. The split between the two panels is POSTURE, not position in the list
// (owner, 2026-08-29): featured = open signups, other = invite-only. Both are
// derived from one registry so a provider cannot fall off both panels by being
// edited in one place.
...
// you on a create screen that then demands a code. So the posture decides
// whether we offer creation, not the advertised capability.
...
// and run it over the real registry from the unit suite.
...
// MEMBERSHIP is the owner's: two open providers and one invite-only, chosen for
// reputation rather than for count (2026-08-27); EuroSky added 2026-08-29.
// The DATA lives in providers.json (the same file croft-pwa ships). Bluebird's
// CSP is per-page static markup, so tests/unit/signin-providers.test.ts holds
// every OAuth-capable page's connect-src in parity with this list instead of a
// build step deriving it — a <meta> CSP cannot allowlist arbitrary PDS hosts,
// but it can allowlist THESE.
