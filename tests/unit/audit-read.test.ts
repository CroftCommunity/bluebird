...
describe('decryptHistory', () => {
  it('decrypts, orders newest-first, and skips records it cannot open', async () => {
    ...
    const older = await buildSealedSearchRecord({ q: 'older', blocked: false, tier: 'open', at: Date.parse('2026-07-14T00:00:00Z') }, sponsor.publicKeyJwk, '2026-07-14T00:00:00Z', 'green');
    const newer = await buildSealedSearchRecord({ q: 'newer', blocked: true, tier: 'discovery', at: Date.parse('2026-07-15T00:00:00Z') }, sponsor.publicKeyJwk, '2026-07-15T00:00:00Z', 'blue');
    // Sealed to a DIFFERENT key → undecryptable with the sponsor's key, must be skipped.
    const foreign = await buildSealedSearchRecord({ q: 'secret', blocked: false, tier: 'open', at: Date.parse('2026-07-16T00:00:00Z') }, other.publicKeyJwk, '2026-07-16T00:00:00Z', 'green');
  ...
  it('Phase 1 tolerant read: uses the inner precise `at` for new records, falls back to record createdAt for old ones', async () => {
    ...
    const older: SealedSearchRecord = { enc: oldBox, createdAt: '2026-07-10T00:00:00.000Z', tier: 'green' };
