...
import { b64urlFromString, sha256B64url, signJwt, decodeJwt } from '../../src/atproto/oauth/jose.js';
import { createPkce, challengeS256 } from '../../src/atproto/oauth/pkce.js';
import {
  generateDpopKey,
  exportDpopKey,
  importDpopKey,
  jwkThumbprint,
  createDpopProof,
} from '../../src/atproto/oauth/dpop.js';
...
describe('jose', () => {
  ...
  it('S256 matches the RFC 7636 appendix B test vector', async () => {
    // verifier "dBjftJeZ4CVP-mB92K27uhbUJU1p1r_wW1gFWFOEjXk" → known challenge.
...
describe('dpop', () => {
  ...
  it('round-trips a key through export/import (survives the redirect)', async () => {
    ...
    // The restored private key can still sign.
  ...
  it('builds a verifiable DPoP proof with the expected claims', async () => {
    ...
    // The signature actually verifies against the embedded public key.
