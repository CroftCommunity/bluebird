// Bluebird build: bundle the TS app with esbuild, cache-bust the entry, and emit
...
const PAGES = [
  ...
  { html: 'patrol.html', entry: 'src/sponsor.ts', token: '%SPONSOR_JS%' },
  { html: 'audit.html', entry: 'src/audit.ts', token: '%AUDIT_JS%' },
  { html: 'locker.html', entry: 'src/saves/page.ts', token: '%SAVES_JS%' },
  { html: 'post.html', entry: 'src/post/page.ts', token: '%POST_JS%' },
  { html: 'my-mountain.html', entry: 'src/mysky/page.ts', token: '%MYSKY_JS%' },
  { html: 'trailmap.html', entry: 'src/telescope/page.ts', token: '%TELESCOPE_JS%' },
  { html: 'help.html', entry: 'src/help.ts', token: '%HELP_JS%' },
  { html: 'ski-school.html', entry: 'src/ski-school.ts', token: '%GUIDE_JS%' },
];
...
const result = await esbuild.build({
  ...
  metafile: true,
  define: {
    __BLUEBIRD_VERSION__: JSON.stringify(version),
  },
});
...
// Static assets copied verbatim into dist. (Note: assets/brand/archive/source/ is NEVER
...
// Guard: asset/nav paths must be RELATIVE, never absolute-root. Bluebird deploys
// to a domain root today (bluebird.croft.ing), but per-PR previews (planned) serve
...
const CACHE = 'bluebird-${version.replace(/[^\w.+-]/g, '_')}';
...
// Bundle-size budget (adopted from croft-pwa). bluebird code-splits, so this caps
